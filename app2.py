...
def highlight_practicals(df):
    ...
    # Iterate to apply individual cell styles
    ...
    # Define general table styles, including black borders
    table_styles = [{'selector': 'th, td', 'props': [('border', '1px solid black')]}]

    # Apply the collected styles to the DataFrame
...
def generate_timetable(uploaded_file, timetable_placeholder, download_button_placeholder, course_details_placeholder):
    ...
    if tables:
        ...
        disp_elec = df[(df['COURSE NO.'].str.startswith('CS') | 
        ...
        if 'constraints' in st.session_state and st.session_state['constraints']:
            ...
            st.info("No section constraints applied")
        ...
        if st.session_state.electives:
            ...
            st.info("No electives chosen/allowed")
        ...
        if compulsory_df is None:
            ...
            if solution:
                st.success("Done building a timetable!")
            else:
            ...
            for course, details in solution.items():
                ...
                if 'Practical' in course:
                    section_number = 'P' + str(section_number)
                     
                else:
                    section_number = 'L' + str(section_number)
                data.append({'Course': course, 'Section': section_number, 'Instructor': instructor_name})
                st.markdown(f"`{course:<35}` Section: `{section_number}` Instructor: `{instructor_name}`")
...
col2.image("calendar.png", use_column_width=True)

col1.markdown("""### Welcome to Your Very Own Timetable Application! ⏰
Exclusively for BITS Students! ✨ Do you find it difficult to understand the academic timetable shared by the college? Or you're just too lazy to find out which electives you can choose? Look no further!

This application provides a dynamic and interactive way to create your own timetable completely based on which discipline, year you're in. Navigate through your schedule, explore different courses, and plan your semester more effectively.

##### Features:

Interactive Timetable: Scroll through your weekly schedule, adjust views, and get detailed information about each course or session.  
Course Details: Click on any course to see detailed information including instructor names, room numbers, and session timings.""")
...
timetable_placeholder = st.empty()  # For the timetable
download_button_placeholder = st.empty()  # For the download button
course_details_placeholder = st.empty()  # For course details
...
if st.session_state.form_submitted is not None and st.session_state.all_elective_df is not None and flag:
    ele = st.session_state.all_elective_df
    if st.session_state.all_elective_df is not None:
        st.subheader("Add complusory electives?")
        st.write(" ⚠️ You have to satisfy one compulsory elective")
        mymsg2 = st.empty()
        electives_title = ele["COURSE TITLE"].unique()
        ...
        if selected_elective:
            ...
            available_sections2 = filtered_df2["SEC"].unique()
...
if st.session_state.form_submitted is not None and st.session_state.hum is not None and flag:
    humanities = st.session_state.hum
    st.subheader("Add humanities electives?")
    st.write("⚠️ You have to satisfy one humanities elective")
    electives_title = humanities["COURSE TITLE"].unique()
...
if st.session_state.form_submitted is not None and st.session_state.compulsory_df is not None and flag:
    ...
    if st.session_state.electives:
        available_courses = merged_df["COURSE TITLE"].unique()
        ...
        if selec_course:
            ...
            available_sections = filtered_df2["SEC"].unique()
            selec_section = st.selectbox("Select a section:", available_sections, key="selec_section")
            
    else:
        available_courses = compul_df["COURSE TITLE"].unique()
        ...
        if selec_course:
            ...
            available_sections = filtered_df["SEC"].unique()
    ...
    if col_2.button("Apply and Generate Timetable"):
        ...
        mymsg.markdown(content)
        st.write("Updated Timetable with constraints will be displayed here...")
        generate_timetable(uploaded_file)